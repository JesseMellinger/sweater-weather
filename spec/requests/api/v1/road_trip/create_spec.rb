...
describe 'road trip' do
  ...
  it 'returns error response with 401 status code if api key is not given (sad path)' do
    payload = {
                "origin": "Denver,CO",
                "destination": "Pueblo,CO",
                "api_key": ''
  ...
  it 'returns error response with 401 status code if api key is incorrect (sad path)' do
    payload = {
                "origin": "Denver,CO",
                "destination": "Pueblo,CO",
                "api_key": rand(1..100)
  ...
  it 'returns blank \'weather_at_eta\' attribute if impossible route (sad path)', :vcr do
    payload = {
                "origin": "New York, NY",
    ...
    expect(endpoint_response[:data][:attributes]).to have_key(:travel_time)
    expect(endpoint_response[:data][:attributes][:travel_time]).to eq("Impossible Route")

    expect(endpoint_response[:data][:attributes]).to have_key(:weather_at_eta)
    expect(endpoint_response[:data][:attributes][:weather_at_eta]).to be_a(Hash)
    expect(endpoint_response[:data][:attributes][:weather_at_eta]).to be_empty
  end
end
